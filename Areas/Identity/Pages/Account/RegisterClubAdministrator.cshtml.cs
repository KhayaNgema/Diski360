...
namespace MyField.Areas.Identity.Pages.Account
{
    public class RegisterClubAdministratorModel : PageModel
    {
        ...
        public async Task OnGetAsync(string returnUrl = null)
        {
            ...
            var clubs = await _context.Club.Where(c => c.DivisionId == divisionId).ToListAsync();

            ViewData["Clubs"] = clubs.Select(c => new SelectListItem
            {
                Value = c.ClubId.ToString(),
                Text = c.ClubName,
                Selected = Input.SelectedClubId == c.ClubId 
            }).ToList();

            if (Input.SelectedClubId == 0 && clubs.Any())
            {
                Input.SelectedClubId = clubs.First().ClubId; 
            }
        }
